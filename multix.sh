...
# ==============================================================================
# MultiX Pro V49.0 (Dual-Stack Enforced)
# 核心修正：强制监听 [::] + 内核映射 | 3X-UI 深度适配 | 三级 UI 逻辑
# ==============================================================================
...
# --- [ V49 核心：双栈环境强制修复 ] ---
fix_dual_stack() {
    echo -e "${YELLOW}⚙️ 正在配置内核双栈监听 (IPv4 映射到 IPv6)...${PLAIN}"
    # 关键：设置 bindv6only=0，让监听 :: 的 Socket 也能接收 IPv4 流量
...
install_dependencies() {
    ...
    fix_dual_stack # 每次安装依赖都强制刷一遍内核参数
}
...
install_master() {
    ...
    echo -e "${YELLOW}🛰️ 部署主控端 (强制 IPv6 双栈监听)...${PLAIN}"
...
<body class="p-8">
    <div id="app">
        <div class="flex flex-col md:flex-row justify-between items-center mb-10 gap-6">
            <div>
                <h1 class="text-4xl font-black text-blue-500 italic uppercase">🛰️ MultiX Pro <span class="text-xs text-zinc-600">Dual-Stack</span></h1>
...
def start_ws():
    ...
    async def m():
        # V49 核心修正：绑定 :: (IPv6 wildcard) + family=AF_INET6 + bindv6only=0
        # 这样才能在 Linux 内核层同时吃掉 v4 和 v6 流量
...
EOF
    ...
    get_public_ips
    echo -e "${GREEN}✅ 主控端部署成功！(内核级双栈已激活)${PLAIN}"
            ...
            async with websockets.connect(uri) as ws:
                ...
                while True:
                    ...
                    for r in cur.fetchall():
                        settings = json.loads(r[4])
                        nodes.append({"id": r[0], "remark": r[1], "port": r[2], "protocol": r[3], "settings": settings})
                    conn.close()
...
main_menu() {
    clear
    echo -e "${SKYBLUE}🛰️ MultiX Pro (V49.0 终极双栈版)${PLAIN}"
    echo "------------------------------------------------"
