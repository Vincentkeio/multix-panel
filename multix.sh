...
# ==============================================================================
# MultiX Pro V50.0 (Syntax Fix & Stability Patch)
# 修复：V49 Python 语法错误 (finally block) | 保持双栈监听 ::
# ==============================================================================
...
credential_center() {
    clear
    echo -e "${SKYBLUE}🔐 凭据管理${PLAIN}"
    if [ -f $M_ROOT/.env ]; then
        source $M_ROOT/.env; get_public_ips
        echo -e "${YELLOW}[主控]${PLAIN} http://[${IPV6}]:${M_PORT} (推荐IPv6)"
...
install_master() {
    ...
    echo -e "${YELLOW}🛰️ 部署主控 (V50 修复版)...${PLAIN}"
...
<html class="dark">
<head>
    <meta charset="UTF-8"><title>MultiX V50</title>
...
async def ws_handler(ws):
    ...
    # V50 修复：finally 必须标准换行，否则 Python 解析器报错
    finally: 
...
install_agent() {
    ...
    echo -e "${YELLOW}连接协议:${PLAIN} 1.自动  2.IPv4  3.IPv6 (推荐)"; read -p "选择: " NET_OPT
    TARGET_HOST="$IN_HOST"
...
asyncio.run(run())
EOF
    cd $M_ROOT/agent; docker build -t multix-agent-v50 .
    docker rm -f multix-agent 2>/dev/null
    docker run -d --name multix-agent --restart always --network host -v /var/run/docker.sock:/var/run/docker.sock -v $M_ROOT/agent/db_data:/app/db_share -v $M_ROOT/agent:/app multix-agent-v50
...
sys_tools() {
    while true; do
        ...
        echo "1. BBR加速 (Chiakge)"; echo "2. 安装 3X-UI (MHSanaei)"; echo "3. 申请 SSL"; echo "4. 重置 3X-UI 账号"; echo "5. 清空流量"; echo "0. 返回"
...
main_menu() {
    clear; echo -e "${SKYBLUE}🛰️ MultiX Pro (V50.0 语法修复版)${PLAIN}"
    echo "--------------------------------"
    echo " 1. 安装 主控端"
    echo " 2. 安装 被控端"
    echo "--------------------------------"
    ...
    echo "--------------------------------"
    echo " 7. 凭据管理"; echo " 8. 实时日志"; echo " 9. 运维工具箱"
    echo " 0. 退出"; read -p "选择: " c
    case $c in
        1) install_master ;; 2) install_agent ;;
        3) read -p "IP: " t; nc -zv -w 5 $tip 8888; pause_back ;;
        ...
        9) sys_tools ;; 0) exit 0 ;; *) main_menu ;;
    esac
}
main_menu
