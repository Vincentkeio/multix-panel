...
# ==============================================================================
# Multiy Pro Script V75.6 (MODULAR FINAL FIX)
# Fix 1: [Init] 启动即建立 multiy 快捷指令
# Fix 2: [Master] 强制清理旧进程，支持自定义 Token 输入交互
# Fix 3: [UI] 面板 Token 强制 API 实时拉取，确保与 env 文件 100% 同步
# Fix 4: [Config] 选项 5 整合完整凭据中心，支持即时信息查看
# ==============================================================================
...
credential_center() {
    ...
    if [ -n "$M_TOKEN" ]; then
        ...
        echo -e "IPv4 登录: ${GREEN}http://${IPV4}:${M_PORT}${PLAIN}"
        echo -e "IPv6 登录: ${GREEN}http://[${IPV6}]:${M_PORT}${PLAIN}"
        echo -e "账号: ${GREEN}${M_USER}${PLAIN} | 密码: ${GREEN}${M_PASS}${PLAIN}"
        echo -e "令牌 (Token): ${YELLOW}${M_TOKEN}${PLAIN}"
    ...
    if [ -f "$M_ROOT/agent/agent.py" ]; then
        ...
        echo -e "${YELLOW}[被控端配置]${PLAIN}"
        echo -e "当前连接目标: ${SKYBLUE}${A_HOST}${PLAIN}"
        echo -e "------------------------------------------------"
    fi
...
# --- [ 模块 3：主控安装 ] ---
install_master() {
    clear; echo -e "${SKYBLUE}>>> 部署 Multiy 主控 (模块化加固版)${PLAIN}"
    ...
    read -p "1. 面板端口 [7575]: " M_PORT; M_PORT=${M_PORT:-7575}
    read -p "2. 通信端口 [9339]: " WS_PORT; WS_PORT=${WS_PORT:-9339}
    read -p "3. 用户名 [admin]: " M_USER; M_USER=${M_USER:-admin}
    read -p "4. 密码 [admin]: " M_PASS; M_PASS=${M_PASS:-admin}
    
    # Token 交互修复块
    DEFAULT_TK=$(cat /dev/urandom | tr -dc 'a-zA-Z0-9' | fold -w 16 | head -n 1)
    echo -e "------------------------------------------------"
    echo -e "建议使用随机令牌: ${YELLOW}${DEFAULT_TK}${PLAIN}"
    ...
    echo -e "${GREEN}✅ 主控部署完成！Token 已同步。${PLAIN}"
    pause_back
}
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <template x-for="(a, ip) in agents" :key="ip">
                <div class="glass border-l-4 border-blue-500">
                    <div class="flex justify-between"><b class="text-lg" x-text="a.alias"></b><span class="w-2 h-2 bg-green-500 rounded-full shadow-[0_0_8px_#22c55e]"></span></div>
                    <div class="text-[10px] text-slate-500 my-4 font-mono" x-text="ip"></div>
...
# --- [ 模块：被控端部署 ] ---
install_agent() {
    clear; echo -e "${SKYBLUE}>>> 部署 Multiy 被控 (WSS 加密版)${PLAIN}"
...
main_menu() {
    ...
    case $c in
        ...
        3) 
            echo -e "${YELLOW}[监听端口]${PLAIN}"; ss -tuln | grep -E "$(get_env_val 'M_PORT')|$(get_env_val 'WS_PORT')"
            echo -e "\n${YELLOW}[被控日志]${PLAIN}"; journalctl -u multiy-agent -f --output cat ;;
