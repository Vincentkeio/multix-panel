#!/bin/bash
# Multiy Pro V98.0 - 旗舰双栈全功能版 (支持永久示例节点)

export M_ROOT="/opt/multiy_mvp"
SH_VER="V98.0"
RED='\033[0;31m'; GREEN='\033[0;32m'; YELLOW='\033[0;33m'; SKYBLUE='\033[0;36m'; PLAIN='\033[0m'

# --- [ 基础工具函数 ] ---
check_root() { [[ $EUID -ne 0 ]] && echo -e "${RED}[错误]${PLAIN} 需 Root 权限!" && exit 1; }
install_shortcut() { [ ! -f /usr/bin/multiy ] && cp "$0" /usr/bin/multiy && chmod +x /usr/bin/multiy; }
get_env_val() { [ -f "$M_ROOT/.env" ] && grep "^$1=" "$M_ROOT/.env" | cut -d"'" -f2 || echo ""; }
pause_back() { echo -e "\n${YELLOW}按任意键返回主菜单...${PLAIN}"; read -n 1 -s -r; main_menu; }

# --- [ 1. 凭据与配置看板 (双栈监测增强版) ] ---
credential_center() {
    clear
    [ ! -f "$M_ROOT/.env" ] && echo -e "${RED}尚未安装主控！${PLAIN}" && pause_back && return
    source "$M_ROOT/.env"
    V4=$(curl -s4m 3 api.ipify.org || echo "未分配")
    V6=$(curl -s6m 3 api64.ipify.org || echo "未分配")
    
    echo -e "${SKYBLUE}==================================================${PLAIN}"
    echo -e "          🛰️  MULTIY PRO 旗舰双栈凭据看板"
    echo -e "${SKYBLUE}==================================================${PLAIN}"
    echo -e "${GREEN}[ 1. 管理入口监测 ]${PLAIN}"
    echo -e " 🔹 IPv4 面板: http://$V4:$M_PORT"
    [ "$V6" != "未分配" ] && echo -e " 🔹 IPv6 面板: http://[$V6]:$M_PORT"
    echo -e " 🔹 管理员: ${YELLOW}$M_USER${PLAIN} / 密码: ${YELLOW}$M_PASS${PLAIN}"
    
    echo -e "\n${GREEN}[ 2. 物理链路监听状态 ]${PLAIN}"
    # 分别检测 V4 和 V6 监听
    if netstat -tuln | grep -q "0.0.0.0:9339"; then V4_S="${GREEN}● ACTIVE${PLAIN}"; else V4_S="${RED}○ OFF${PLAIN}"; fi
    if netstat -tuln | grep -q ":::9339"; then V6_S="${GREEN}● ACTIVE${PLAIN}"; else V6_S="${RED}○ OFF${PLAIN}"; fi
    echo -e " 🔹 9339 隧道 (IPv4): $V4_S"
    echo -e " 🔹 9339 隧道 (IPv6): $V6_S"
    
    echo -e "\n${GREEN}[ 3. 被控连接核心凭据 ]${PLAIN}"
    echo -e " 🔹 通信 Token: ${YELLOW}$M_TOKEN${PLAIN}"
    echo -e " 🔹 建议域名: $M_HOST (支持双栈解析)"
    echo -e "${SKYBLUE}==================================================${PLAIN}"
    pause_back
}

# --- [ 2. 主控安装逻辑 ] ---
install_master() {
    clear; echo -e "${SKYBLUE}>>> 部署 Multiy 旗舰主控 (V98.0)${PLAIN}"
    pkill -9 -f "app.py" 2>/dev/null
    
    # 强力环境校准与 SSL 修复
    apt-get update && apt-get install -y python3 python3-pip openssl curl net-tools >/dev/null 2>&1
    pip3 install "Flask<3.0.0" "python-socketio" "eventlet==0.33.3" "psutil" "cryptography==38.0.4" "pyOpenSSL==22.1.0" --break-system-packages --user >/dev/null 2>&1

    mkdir -p "$M_ROOT/master"
    openssl req -x509 -newkey rsa:2048 -keyout "$M_ROOT/master/key.pem" -out "$M_ROOT/master/cert.pem" -days 3650 -nodes -subj "/CN=Multiy" >/dev/null 2>&1

    read -p "面板 Web 端口 [7575]: " M_PORT; M_PORT=${M_PORT:-7575}
    read -p "管理用户名 [admin]: " M_USER; M_USER=${M_USER:-admin}
    read -p "管理密码 [admin]: " M_PASS; M_PASS=${M_PASS:-admin}
    TK_RAND=$(openssl rand -base64 12 | tr -d '/+=')
    read -p "通信 Token (回车用 $TK_RAND): " IN_TK; M_TOKEN=${IN_TK:-$TK_RAND}
    M_HOST="multix.spacelite.top"

    echo -e "M_TOKEN='$M_TOKEN'\nM_PORT='$M_PORT'\nM_USER='$M_USER'\nM_PASS='$M_PASS'\nM_HOST='$M_HOST'" > "$M_ROOT/.env"
    
    _generate_master_py
    _deploy_service "multiy-master" "$M_ROOT/master/app.py"
    
    if command -v ufw >/dev/null; then ufw allow 9339/tcp; ufw allow "$M_PORT"/tcp; ufw reload; fi
    
    echo -e "${GREEN}✅ 主控已拉起，正在跳转旗舰看板...${PLAIN}"
    sleep 2; credential_center
}

_generate_master_py() {
cat > "$M_ROOT/master/app.py" << 'EOF'
import socketio, eventlet, os, json, ssl, time, psutil, subprocess
from flask import Flask, render_template_string, session, redirect, request, jsonify
from threading import Thread

def load_env():
    c = {}
    path = '/opt/multiy_mvp/.env'
    if os.path.exists(path):
        with open(path, encoding='utf-8') as f:
            for l in f:
                if '=' in l: k,v = l.strip().split('=', 1); c[k] = v.strip("'\"")
    return c

conf = load_env()
sio = socketio.Server(cors_allowed_origins='*', async_mode='eventlet', ping_timeout=20, ping_interval=5)
app = Flask(__name__)
app.wsgi_app = socketio.WSGIApp(sio, app.wsgi_app)
AGENTS = {}

@sio.on('auth')
def authenticate(sid, data):
    env = load_env()
    if data.get('token') == env.get('M_TOKEN'):
        AGENTS[sid] = {
            "alias": data.get('hostname', 'Node'),
            "stats": {"cpu":0,"mem":0},
            "last_seen": time.time(),
            "ip": request.remote_addr,
            "connected_at": time.strftime("%H:%M:%S")
        }
        sio.emit('ready', {'msg': 'verified'}, room=sid)
        sio.emit('update_ui', AGENTS)
        return True
    return False

@sio.on('heartbeat')
def handle_heartbeat(sid, data):
    if sid in AGENTS:
        AGENTS[sid]['stats'] = data
        AGENTS[sid]['last_seen'] = time.time()
        sio.emit('update_ui', AGENTS)

@sio.on('disconnect')
def disconnect(sid):
    if sid in AGENTS: 
        del AGENTS[sid]
        sio.emit('update_ui', AGENTS)

@app.route('/api/info')
def api_info():
    env = load_env()
    ip4 = subprocess.getoutput("curl -s4m 2 api.ipify.org || echo 'N/A'")
    ip6 = subprocess.getoutput("curl -s6m 2 api64.ipify.org || echo 'N/A'")
    return jsonify({
        "token": env.get('M_TOKEN'),
        "ip4": ip4, "ip6": ip6,
        "m_port": env.get('M_PORT'),
        "w_port": "9339"
    })

@app.route('/api/state')
def api_state():
    return jsonify({"agents": AGENTS})

# --- [ 旗舰美化前端：含永久示例节点 & 顶栏信息 ] ---
INDEX_HTML = """
<!DOCTYPE html><html><head><meta charset="UTF-8"><script src="https://cdn.tailwindcss.com"></script>
<script defer src="https://cdn.jsdelivr.net/npm/alpinejs@3.x.x/dist/cdn.min.js"></script>
<script src="https://cdn.socket.io/4.5.4/socket.io.min.js"></script>
<style>
    body{background:#020617;color:#fff;font-family:ui-sans-serif,system-ui}
    .glass{background:rgba(255,255,255,0.02);backdrop-filter:blur(15px);border:1px solid rgba(255,255,255,0.08);border-radius:2rem}
    .progress-bar{height:6px;border-radius:10px;background:rgba(255,255,255,0.05);overflow:hidden}
    .top-badge{background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.05);padding:8px 16px;border-radius:12px;font-size:10px;font-weight:900}
</style></head>
<body class="p-6 md:p-12" x-data="panel()" x-init="init()">
    <div class="max-w-7xl mx-auto">
        <div class="flex flex-wrap gap-4 mb-10">
            <div class="top-badge">TOKEN: <span class="text-blue-400" x-text="sys.token"></span></div>
            <div class="top-badge">IPV4: <span class="text-blue-400" x-text="sys.ip4 + ':' + sys.m_port"></span></div>
            <div class="top-badge">IPV6: <span class="text-blue-400" x-text="'['+sys.ip6+']:' + sys.m_port"></span></div>
            <div class="top-badge">WSS: <span class="text-green-500">9339 ACTIVE</span></div>
        </div>

        <div class="flex justify-between items-end mb-16">
            <div>
                <h1 class="text-6xl font-black italic tracking-tighter text-blue-500">MULTIY <span class="text-white">PRO</span></h1>
                <p class="text-slate-500 font-bold text-xs uppercase mt-2 tracking-widest">Dual-Stack Infrastructure Dashboard</p>
            </div>
            <a href="/logout" class="bg-red-500/10 text-red-500 px-8 py-3 rounded-2xl text-xs font-black border border-red-500/20 hover:bg-red-500 hover:text-white transition">LOGOUT</a>
        </div>

        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            <div class="glass p-10 border-blue-500/20 bg-blue-500/5">
                <div class="flex justify-between mb-10">
                    <div>
                        <h2 class="text-2xl font-black text-blue-400">DEMO-NODE-01</h2>
                        <code class="text-[10px] text-slate-500 font-bold">SIMULATED (DEBUG MODE)</code>
                    </div>
                    <span class="text-[9px] bg-blue-500/20 px-4 py-2 rounded-xl h-fit font-bold">PERMANENT</span>
                </div>
                <div class="space-y-8">
                    <div>
                        <div class="flex justify-between text-[10px] uppercase font-black mb-2 text-slate-500"><span>Processor</span><span class="text-blue-400" x-text="demoCpu+'%'"></span></div>
                        <div class="progress-bar"><div class="h-full bg-blue-500 transition-all duration-1000" :style="'width:'+demoCpu+'%'"></div></div>
                    </div>
                    <div>
                        <div class="flex justify-between text-[10px] uppercase font-black mb-2 text-slate-500"><span>Memory</span><span class="text-purple-400">42%</span></div>
                        <div class="progress-bar"><div class="h-full bg-purple-500" style="width:42%"></div></div>
                    </div>
                </div>
                <div class="mt-10 pt-6 border-t border-white/5 text-[9px] font-black text-slate-600 uppercase tracking-widest">Status: Testing Environment</div>
            </div>

            <template x-for="(node, sid) in agents" :key="sid">
                <div class="glass p-10 hover:border-blue-500/50 transition-all duration-300">
                    <div class="flex justify-between mb-10">
                        <div>
                            <h2 class="text-2xl font-black" x-text="node.alias"></h2>
                            <code class="text-[10px] text-blue-400 font-bold" x-text="node.ip"></code>
                        </div>
                        <span class="text-[9px] bg-white/5 px-4 py-2 rounded-xl h-fit font-bold" x-text="node.connected_at"></span>
                    </div>
                    <div class="space-y-8">
                        <div>
                            <div class="flex justify-between text-[10px] uppercase font-black mb-2 text-slate-500"><span>Processor</span><span class="text-blue-400" x-text="node.stats.cpu+'%'"></span></div>
                            <div class="progress-bar"><div class="h-full bg-blue-500 transition-all duration-700" :style="'width:'+node.stats.cpu+'%'"></div></div>
                        </div>
                        <div>
                            <div class="flex justify-between text-[10px] uppercase font-black mb-2 text-slate-500"><span>Memory</span><span class="text-purple-400" x-text="node.stats.mem+'%'"></span></div>
                            <div class="progress-bar"><div class="h-full bg-purple-500 transition-all duration-700" :style="'width:'+node.stats.mem+'%'"></div></div>
                        </div>
                    </div>
                </div>
            </template>
        </div>
    </div>
    <script>
        function panel(){ return { 
            agents:{}, sys:{}, demoCpu: 25,
            init(){
                this.socket = io();
                this.socket.on('update_ui', (data) => { this.agents = data; });
                this.fetchData();
                setInterval(() => { this.demoCpu = Math.floor(Math.random() * 20) + 15; }, 3000);
            },
            async fetchData(){
                try{
                    const r = await fetch('/api/state'); const d = await r.json(); this.agents = d.agents;
                    const i = await fetch('/api/info'); const si = await i.json(); this.sys = si;
                }catch(e){}
            } 
        } }
    </script>
</body></html>
"""

@app.route('/login', methods=['GET', 'POST'])
def login():
    env = load_env(); app.secret_key = env.get('M_TOKEN')
    if request.method == 'POST':
        if request.form.get('u') == env.get('M_USER') and request.form.get('p') == env.get('M_PASS'):
            session['logged'] = True; return redirect('/')
    return """<body style="background:#020617;display:flex;justify-content:center;align-items:center;height:100vh;color:#fff;font-family:sans-serif"><form method="post" style="background:rgba(255,255,255,0.03);backdrop-filter:blur(20px);padding:60px;border-radius:35px;border:1px solid rgba(255,255,255,0.1);width:340px;text-align:center"><h2 style="color:#3b82f6;font-size:2rem;font-weight:900;margin-bottom:40px;font-style:italic;letter-spacing:-2px">MULTIY <span style="color:#fff">PRO</span></h2><input name="u" placeholder="Admin" style="width:100%;padding:15px;margin:12px 0;background:#000;border:1px solid #333;color:#fff;border-radius:15px;outline:none"><input name="p" type="password" placeholder="Pass" style="width:100%;padding:15px;margin:12px 0;background:#000;border:1px solid #333;color:#fff;border-radius:15px;outline:none"><button style="width:100%;padding:16px;background:#3b82f6;color:#fff;border:none;border-radius:15px;font-weight:900;cursor:pointer;margin-top:20px">ACCESS SYSTEM</button></form></body>"""

@app.route('/')
def index():
    if not session.get('logged'): return redirect('/login')
    return render_template_string(INDEX_HTML)

def run_wss():
    # 物理监听 IPv6 [::]，支持双栈
    eventlet.wsgi.server(eventlet.wrap_ssl(eventlet.listen(('::', 9339)), 
                         certfile='cert.pem', keyfile='key.pem', server_side=True), app)

if __name__ == '__main__':
    Thread(target=run_wss, daemon=True).start()
    env = load_env()
    app.run(host='::', port=int(env.get('M_PORT', 7575)))
EOF
}

# --- [ 其余模块保持不变 ] ---
_deploy_service() {
    local NAME=$1; local EXEC=$2
    BODY="[Unit]\nDescription=${NAME}\nAfter=network.target\n[Service]\nExecStart=/usr/bin/python3 ${EXEC}\nRestart=always\nWorkingDirectory=$(dirname ${EXEC})\nEnvironment=PYTHONUNBUFFERED=1\n[Install]\nWantedBy=multi-user.target"
    echo -e "$BODY" > "/etc/systemd/system/${NAME}.service"
    systemctl daemon-reload; systemctl enable "${NAME}"; systemctl restart "${NAME}"
}

smart_diagnostic() {
    clear; echo -e "${SKYBLUE}🔍 智能链路诊断中心 (V98.0修复版)${PLAIN}"
    if [ -f "$M_ROOT/agent/agent.py" ]; then
        A_HOST=$(grep "MASTER =" "$M_ROOT/agent/agent.py" | cut -d'"' -f2)
        echo -e "目标地址: ${SKYBLUE}$A_HOST:9339${PLAIN}"
        if curl -sk --max-time 3 "https://$A_HOST:9339" >/dev/null 2>&1 || [ $? -eq 52 ]; then
            echo -e "👉 隧道检测: ${GREEN}成功 (双栈 WSS 响应正常)${PLAIN}"
        else
            echo -e "👉 隧道检测: ${RED}失败 (请检查主控 IPv6 监听及安全组)${PLAIN}"
        fi
    fi
    pause_back
}

main_menu() {
    clear; echo -e "${SKYBLUE}🛰️ Multiy Pro ${SH_VER}${PLAIN}"
    echo " 1. 物理安装/修复主控 (双栈旗舰版)"
    echo " 2. 安装/更新被控 (自动双栈适配)"
    echo " 3. 智能链路诊断中心"
    echo " 4. 凭据与配置看板 (全方位监测)"
    echo " 5. 深度清理中心"
    echo " 0. 退出"
    read -p "选择: " c
    case $c in 1) install_master ;; 2) install_agent ;; 3) smart_diagnostic ;; 4) credential_center ;; 5) rm -rf $M_ROOT; exit ;; 0) exit ;; esac
}

check_root; install_shortcut; main_menu
