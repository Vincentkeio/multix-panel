...
# ==============================================================================
# Multiy Pro Script V75.1 (COMPLETE STABLE VERSION)
# 1. [Init] 启动即创建 multiy 快捷指令
# 2. [Master] 修复交互：强制显示建议 Token 并等待用户输入
# 3. [Sync] 彻底解决 Token 不一致，面板与配置文件物理同步
# 4. [Net] 强制双协议栈物理监听，解决 9339 端口不亮问题
# ==============================================================================
...
check_root() { [[ $EUID -ne 0 ]] && echo -e "${RED}[错误]${PLAIN} 请使用 root 权限运行！" && exit 1; }
get_public_ips() { 
...
# --- [ 模块 2：凭据与配置中心 ] ---
credential_center() {
    clear; echo -e "${SKYBLUE}🔐 Multiy 凭据与配置中心 (V75.1)${PLAIN}"
    M_TOKEN=$(get_env_val "M_TOKEN")
    M_PORT=$(get_env_val "M_PORT")
    WS_PORT=$(get_env_val "WS_PORT")
    M_USER=$(get_env_val "M_USER")
    ...
    if [ -n "$M_TOKEN" ]; then
        ...
        echo -e "管理员账号: ${GREEN}${M_USER}${PLAIN}"
    ...
    if [ -f "$M_ROOT/agent/agent.py" ]; then
        ...
        echo -e "${YELLOW}[被控端 - 连接状态]${PLAIN}"
        echo -e "目标主机: ${SKYBLUE}${A_HOST}${PLAIN}"
        echo -e "通信端口: ${SKYBLUE}${A_PORT}${PLAIN}"
        echo -e "------------------------------------------------"
    fi
    echo " 1. 重新安装主控 | 2. 重新安装被控 | 0. 返回"
    read -p "选择: " c_opt
    case $c_opt in
        1) install_master ;;
        2) install_agent ;;
...
# --- [ 模块 3：主控端部署 ] ---
install_master() {
    clear; echo -e "${SKYBLUE}>>> 部署 Multiy 主控端 (Jinja2 隔离版)${PLAIN}"
    # 依赖安装
    ...
    # 参数交互
    read -p "1. 面板 Web 访问端口 [7575]: " M_PORT; M_PORT=${M_PORT:-7575}
    read -p "2. 小鸡通信监听端口 [9339]: " WS_PORT; WS_PORT=${WS_PORT:-9339}
    read -p "3. 后台管理用户名 [admin]: " M_USER; M_USER=${M_USER:-admin}
    read -p "4. 后台管理密码 [admin]: " M_PASS; M_PASS=${M_PASS:-admin}
    
    # --- Token 交互修复块 ---
    DEFAULT_TK=$(cat /dev/urandom | tr -dc 'a-zA-Z0-9' | fold -w 16 | head -n 1)
    echo -e "------------------------------------------------"
    echo -e "建议使用生成的随机 Token: ${YELLOW}${DEFAULT_TK}${PLAIN}"
    read -p "输入自定义 Token (直接回车则使用建议值): " IN_TOKEN
    M_TOKEN=${IN_TOKEN:-$DEFAULT_TK}
    echo -e "已确认 Token: ${GREEN}${M_TOKEN}${PLAIN}"
    echo -e "------------------------------------------------"
    ...
    echo -e "${GREEN}✅ 主控部署成功！请进入“凭据中心”查看详情。${PLAIN}"
    pause_back
}
...
def index():
    ...
    <style>body{background:#020617;color:#fff}.glass{background:rgba(15,23,42,0.8);backdrop-filter:blur(20px);border:1px solid rgba(255,255,255,0.1);border-radius:24px}</style></head>
    <body class="p-10" x-data="panel()" x-init="start()">
        <div class="flex justify-between items-center mb-10">
            ...
            <div class="flex gap-4">
                ...
                <button @click="fetchData()" class="text-xs bg-blue-600 px-4 py-2 rounded-full">REFRESH</button>
            </div>
        </div>
...
def login():
    ...
    <form method="post" style="background:rgba(255,255,255,0.05);backdrop-filter:blur(15px);padding:50px;border-radius:24px;width:300px;text-align:center">
        <h2 style="color:#3b82f6;font-weight:900">Multiy Login</h2>
        <input name="u" placeholder="Admin" style="width:100%;padding:12px;margin:10px 0;background:#000;border:1px solid #333;color:#fff;border-radius:10px">
        <input name="p" type="password" placeholder="Pass" style="width:100%;padding:12px;margin:10px 0;background:#000;border:1px solid #333;color:#fff;border-radius:10px">
        <button style="width:100%;padding:12px;background:#3b82f6;color:#fff;border:none;border-radius:10px;font-weight:bold;cursor:pointer">ENTER</button>
    </form></body>"""
...
if __name__ == '__main__':
    ...
    conf = load_env(); app.run(host='::', port=int(conf.get('M_PORT')))
EOF
}

# --- [ 模块 4：被控端部署 ] ---
install_agent() {
    clear; echo -e "${SKYBLUE}>>> 部署 Multiy 被控端${PLAIN}"
    mkdir -p "$M_ROOT/agent"
    read -p "1. 主控域名或 IP: " M_HOST
    read -p "2. 主控通信端口 [9339]: " WS_PORT; WS_PORT=${WS_PORT:-9339}
    read -p "3. 主控 Token: " M_TOKEN
    echo -e "偏好: 1. 强制 IPv6 (适合 NAT 小鸡) | 2. 强制 IPv4 | 3. 自动"
...
main_menu() {
    ...
    echo " 1. 安装/更新 Multiy 主控 (自定义 Token)"
    echo " 2. 安装/更新 Multiy 被控 (WSS 加密)"
    echo " 3. 连接监控中心 (查看端口监听/日志)"
    echo " 4. 凭据与配置中心 (主/被控全信息查看)"
    echo " 5. 卸载清理组件"
    echo " 0. 退出"
    read -p "选择: " c
    case $c in
        ...
        4) credential_center ;;
        5) systemctl stop multiy-master multiy-agent; rm -rf "$M_ROOT" /etc/systemd/system/multiy-*; echo "已清理"; exit 0 ;;
        0) exit 0 ;; *) main_menu ;;
    esac
}
